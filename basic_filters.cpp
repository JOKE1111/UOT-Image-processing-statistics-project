...
vector <Pixel> runMask(const vector <double> &mask, const vector <double> &SobelMask, const vector <Pixel> &image, int kernelSize, int WIDTH, int HEIGHT); 
...
vector <Pixel> Blur(const vector <Pixel> &image, int WIDTH, int HEIGHT) // the blur function that does the basic and gaussian blur
{
...
void makeFile(const vector <Pixel> &image, int WIDTH, int HEIGHT) // this function is given a flat vector and it constructs a bmp file
{
    separator();
